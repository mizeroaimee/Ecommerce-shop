...
class DessertShopUI {
    ...
    setupEventListeners() {
        ...
        this.orderModal.addEventListener("click", (e) => {
            if (e.target === this.orderModal || e.target.classList.contains("modal-overlay")) {
                this.closeModal();
            }
        });
    }
    ...
    updateDessertCard(dessertId) {
        const card = document.querySelector(`[data-dessert-id="${dessertId}"]`);
        if (!card)
            return;
